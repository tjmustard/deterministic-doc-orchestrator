...
1. Run `/discover` — scans the codebase and populates `spec/compiled/architecture.yml`.
2. Review the generated YAML for accuracy.
3. Run `/baseline` — generates the initial `SuperPRD.md`.
...
1. Run `/architect` — begins the requirements extraction interview (creates `Draft_PRD.md` in `spec/active/`).
2. **Start a new conversation** and run `/redteam` — adversarial analysis (creates `RedTeam_Report.md` in `spec/active/`).
3. **Start a new conversation** and run `/resolve` — resolves trade-offs and compiles `SuperPRD.md` and `MiniPRD` files into `spec/compiled/`.
...
3. **Start a new conversation** and run `/audit spec/compiled/MiniPRD_[Target].md` to verify code against the contract.
