...
- When the user wants to brainstorm a new feature before running `/architect`.
...
- When the user explicitly runs `/consult-cto`.
...
3. **Decision Point:**
   - **If the feature warrants a full specification:** Instruct the user to run `/architect` to formally draft the spec and begin the state-machine pipeline.
