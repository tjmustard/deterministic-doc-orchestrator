...
- When the user explicitly runs `/prompt-engineer`.
...
5. **Finalization & Export**
   ...
   - Ask if they want to export the prompt as a reusable component:
     - **`/new-workflow`**: Convert to a simple slash command workflow.
     - **`/create-skill`**: Deeply integrate as a full agentic skill with `SKILL.md`, `scripts/`, and `examples/`.
