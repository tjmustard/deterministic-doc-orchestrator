...
- When the user explicitly runs `/hyper-refresh-memory`.
...
5. **Sync Documentation**
   ...
   - **CRITICAL:** Do NOT manually edit `architecture.yml`. If the Hypergraph is out of sync with reality, instruct the user to run `/hyper-discover` or `/hyper-audit` to deterministically reconcile it.
